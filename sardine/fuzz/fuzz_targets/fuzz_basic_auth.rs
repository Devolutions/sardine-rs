...
extern crate sardine;

use sardine::{Srd, Cipher, blobs::BasicBlob};
...
fuzz_target!(|data: &[u8]| {
    ...
    let basic_blob = BasicBlob::new("username", "password");
    ...
    while !(client_status && server_status) {
        client_status = match client.authenticate(&data, &mut out_data) {
            Ok(status) => status,
            Err(_) => break,
        };
        out_data = Vec::new();

        server_status = match server.authenticate(&data, &mut out_data) {
            Ok(status) => status,
            Err(_) => break,
        };
        out_data = Vec::new();
